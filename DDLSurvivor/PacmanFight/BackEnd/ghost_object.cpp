...
int max_cnt=getRandom(300, 450);
int who_first=max_cnt%2;//0red1blue
QString ghost_path{":/images/resources/ghost.png"};

ghost_object::ghost_object(const QPointF& _pos,qreal _angle):
    GameAbstractObject(ghost_size, ghost_path, ghost, _pos, 0.02,_angle),
    Ghost(_pos, 30,_angle)
{
    setZValue(10);
}
qreal ghost_object::chooseAngle(const QPointF& red, const QPointF& blue){
    cnt++;
...
void ghost_object::set_angle(const QPointF& red, const QPointF& blue){
